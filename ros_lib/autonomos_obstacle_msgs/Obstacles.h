#ifndef _ROS_autonomos_obstacle_msgs_Obstacles_h
#define _ROS_autonomos_obstacle_msgs_Obstacles_h
...
namespace autonomos_obstacle_msgs
{
  ...
  class Obstacles : public ros::Msg
  {
    public:
      ...
      typedef autonomos_obstacle_msgs::Obstacle _obstacles_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(obstacles_lengthT > obstacles_length)
        this->obstacles = (autonomos_obstacle_msgs::Obstacle*)realloc(this->obstacles, obstacles_lengthT * sizeof(autonomos_obstacle_msgs::Obstacle));
      obstacles_length = obstacles_lengthT;
      for( uint32_t i = 0; i < obstacles_length; i++){
      offset += this->st_obstacles.deserialize(inbuffer + offset);
        memcpy( &(this->obstacles[i]), &(this->st_obstacles), sizeof(autonomos_obstacle_msgs::Obstacle));
      }
     return offset;
    }

    const char * getType(){ return "autonomos_obstacle_msgs/Obstacles"; };
    const char * getMD5(){ return "b4b34fb416ab3b0f5be56f7e589cb11d"; };
